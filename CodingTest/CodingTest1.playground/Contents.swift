...
func solution(_ topping:[Int], _ tasting:[Int]) -> Int {
    var fakeToppingArr: [Int] = [Int](repeating: 0, count: topping.count)
    var fakeTastingArr: [Int] = [Int](repeating: 0, count: tasting.count)
    var count = 0
    var now = 0
    for i in 1...topping.count {
        fakeToppingArr[i - 1] = topping[i - 1]
    }

    for i in 1...tasting.count {
        fakeTastingArr[i - 1] = tasting[i - 1]
    }
    
    for j in 0..<fakeTastingArr.count {
        var temp = 1 + j
        var temp2 = 1 - j
        
        if temp > topping.count {
            temp -= topping.count
        }
        if temp2 <= 0 {
            temp2 += topping.count - 1
        }
        if fakeToppingArr[temp] == fakeTastingArr[0] {
            var temp3 = now
            now = temp
            count += j
        } else {
        ...
        if fakeToppingArr[temp2] == fakeTastingArr[0] {
